...
void init_rotor() {
    ...
    if(!rot.theta)
    	rot.theta = 98*M_PI/180.;		// Polar angle
    if(!rot.phi)
...
void rotor_update() {
    ...
    rot.cu = pow(3*rot.Prho*rot.W*rho_a, 1./3.);
}
    ...
    foreach (noauto)
    {
        foreach_dimension(){
            if (fan[] > 0.){
                ...
                damp = (rot.start + rot.rampT) > t ? (t - rot.start) / rot.rampT : 1.;
                corrP = rot.diaVol > 0. ? rot.V / rot.diaVol : 1.;
                w = wsgn * fan[] * damp * sq(rot.nf.x) * (2. / rho_a) * (corrP * rot.P / rot.V) * dt;
                tempW += dv() * fabs(w) / 2;
                // New kinetic energy
                utemp = sq(u.x[]) + w;
