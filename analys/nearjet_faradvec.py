...
files_B = sorted(glob.glob(file_dir + "/HS/PAV_B025_*.npy"))
files_U = sorted(glob.glob(file_dir + "/HS/PAV_M025_*.npy"))
files_TV = sorted(glob.glob(file_dir + "/HS/PAV_TV025_*.npy"))
# %%
B_HS_OP = np.load(files_B[0])*273.15/9.81
B_HS_RE = np.load(files_B[1])*273.15/9.81
...
ax1.plot(xb_along, TV2_OP_along, 'r--', label=r'$\mathrm{T_v}$ on',linewidth = 2)
ax1.plot(xb_along, TV2_RE_along, 'b--', label=r'$\mathrm{T_v}$ off',linewidth = 2)
...
ax2.plot(xb_across, b2_OP_across, 'r-', label='Ta on',linewidth = 2)
ax2.plot(xb_across, b2_RE_across, 'b-', label='Ta off',linewidth = 2)
ax2.plot(xb_across, TV2_OP_across, 'r--', label='Tv on',linewidth = 2)
ax2.plot(xb_across, TV2_RE_across, 'b--', label='Tv off',linewidth = 2)
ylabel_ax1 = np.arange(1,8,2)
...
xb_across = np.arange(2049)-1024
fig, (ax1, ax2) = plt.subplots(2, 1, figsize = (10, 6), sharex=True)
ax1.plot(xb_across, b2_OP_across, 'r-', label='Ta on',linewidth = 2)
ax1.plot(xb_across, b2_RE_across, 'b-', label='Ta off',linewidth = 2)
ax1.plot(xb_across, TV2_OP_across, 'r--', label='Tv on',linewidth = 2)
ax1.plot(xb_across, TV2_RE_across, 'b--', label='Tv off',linewidth = 2)
ylabel_ax1 = np.arange(1,8,2)
