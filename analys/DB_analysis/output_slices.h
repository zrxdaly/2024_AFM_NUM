...
trace void output_Zslice(struct sOutputSlice p)
{
    if (!p.list)
        p.list = all;
    if (p.n == 0)
...
trace void output_Zslice_TV(struct sOutputSlice p)
{
    if (!p.list)
        p.list = all;
    if (p.n == 0)
...
trace void output_W12(struct sOutputSlice p)
{
    ...
    // the W12 output (11 * 11): i = y (0..10),  j = z (990.5...1100.5), x = 528.5
    for (int i = 0; i < p.n; i++)
    {
        double varCoord1 = Delta * i + 0.5; // some clever way of implementing general variation of coordinates instead of mapping them directly to x, y or z
        bool varX = !(p.plane.x < 1.);
        double x = (!varX ? p.plane.x * L0 : varCoord1) + X0;

        for (int j = 0; j < p.n; j++)
        {
            double varCoord2 = Delta * (j + 990.5);
            double y = (varX ? (p.plane.y < 1. ? p.plane.y * L0 : varCoord2) : varCoord1) + Y0;
            double z = (p.plane.z < 1. ? p.plane.z * L0 : varCoord2) + Z0;
            if (p.linear)
            {
